...
#pragma omp parallel for schedule(dynamic)  private(iz,ip) default(shared)	    
	   for (iz = 0; iz < particle.n_zgrid; ++iz)
           {
	    for (ip = 0; ip < particle.n_pgrid; ++ip)
            {
	  ...
	  // NB iz must not be private or wrong result
	   ...
	   iz = 0;
#pragma omp parallel for schedule(dynamic)  private(ir,ip) default(shared)	    
	   for (ir = 0; ir < particle.n_rgrid; ++ir)
           {
	    for (ip = 0; ip < particle.n_pgrid; ++ip)
            {
...
#pragma omp parallel for schedule(dynamic)  private(ir,ip) default(shared)	    
	...
	if (method == 2) // explicit updating method AWS20110118
	{
	  ...
	  int nrzp =  particle.n_rgrid *  particle.n_zgrid *  particle.n_pgrid;
          ...
          if (turbo==0)
	  { 
#pragma omp parallel for schedule(dynamic)  private(ir,iz,ip) default(shared)
	  for (ir = 0; ir < particle.n_rgrid  ; ++ir)
          {
	   for (iz = 0; iz < particle.n_zgrid  ; ++iz)
           {
...
#pragma omp parallel for schedule(dynamic)  private(ir,ip) default(shared)	    
	 ...
	 { 
	  ...
	  // r direction -----------------------------------------------------------
	    ...
	    #pragma omp parallel for schedule(dynamic)  private(ir,iz,ip,irzp,iirzp) default(shared) 
	  ...
	  for (ir = 0; ir < particle.n_rgrid  ; ++ir)
	   ...
	   #pragma omp parallel for schedule(dynamic)   default(shared)
	    ...
	    /* AWS20110201
	  #pragma omp parallel for schedule(dynamic)   default(shared) private(iz,ip,irzp)	  
	  for (ir = 0; ir < particle.n_rgrid  ; ++ir)
          {
	   for (iz = 0; iz < particle.n_zgrid  ; ++iz)
           {
	  ...
	  // NB ir must not be private or wrong result
	   ...
	   ir = particle.n_rgrid-1;
#pragma omp parallel for schedule(dynamic)  private(iz,ip,irzp) default(shared)	    
	   for (iz = 0; iz < particle.n_zgrid; ++iz)
           {
	    for (ip = 0; ip < particle.n_pgrid; ++ip)
            {
	  ...
	  // NB iz must not be private or wrong result
	   ...
	   iz = 0;
#pragma omp parallel for schedule(dynamic)  private(ir,ip,irzp) default(shared)	    
	   for (ir = 0; ir < particle.n_rgrid; ++ir)
           {
	    for (ip = 0; ip < particle.n_pgrid; ++ip)
            {
...
#pragma omp parallel for schedule(dynamic)   default(shared) private(ir,iz,ip,irzp)	  
	  for (ir = 0; ir < particle.n_rgrid  ; ++ir)
          {
	   for (iz = 0; iz < particle.n_zgrid  ; ++iz)
           {
  ...
  if (3 == particle.n_spatial_dimensions) {             // ==== 3D ====
    ...
    */
    ...
    // modification in propagation equation if diffusion coefficient depends on z,R
#pragma omp parallel for schedule(dynamic) default(shared) private(ix,iy,iz,ip)
...
// DIFFUSIVE REACCELERATION   IMOS20020329
    ...
    */
    ...
    uint32_t timestep_mode(1), timestep_mode2done(0);
	...
	for (irept = 1; irept <= nrept; ++irept) {  //AWS20111018
	  ...
	  if (method == 1) {// Crank-Nicolson method     AWS20111018
	    ...
	    if (2 == timestep_mode) {
	      ...
	      INFO(buf.str());
	      std::for_each(galdef.source_classes.begin(), galdef.source_classes.end(), [&](const std::unique_ptr<SourceClass> &sc) { sc->addSource(particle, t/year2sec); });
	      total_source_function  = particle.primary_source_function;
	      total_source_function += particle.secondary_source_function;
	      total_source_function *= dt;                                        //AWS20110131 for consistency with alpha
		...
		buf.str("");
		buf<<"primary source function with time dependence for particle " <<particle.name;
		DEBUGLOG(buf.str());
...
#pragma omp parallel default(shared) private(iz,ix,iy,ip)
	    if (1 == galdef.prop_x) {
	      ...
	      std::valarray<double> Nx0(0., particle.n_xgrid);
	      std::valarray<double> Nx1(0., particle.n_xgrid);
	      std::valarray<double> Nx2(0., particle.n_xgrid);
	      std::valarray<double> Nx3(0., particle.n_xgrid);
	      std::valarray<double> Rx (0., particle.n_xgrid);
...
//for (auto iyz = 0; iyz < particle.n_ygrid*particle.n_zgrid; ++iyz) {

	      //auto iy = iyz / particle.n_zgrid; 
	      //auto iz = iyz % particle.n_zgrid;
		  ...
		  if (galdef.use_symmetry <= 1 || 
		    ...
		    for (ix = 0; ix < particle.n_xgrid; ++ix) {
		      ...
		      //for (auto ipx = 0; ipx < particle.n_pgrid*particle.n_xgrid; ++ipx) {

		      //auto ip = ipx / particle.n_xgrid;
		      //auto ix = ipx % particle.n_xgrid;
...
#pragma omp parallel default(shared) private(iz,ix,iy,ip)
	    if (1 == galdef.prop_y) {
	      ...
	      std::valarray<double> Ny0(0., particle.n_ygrid);
	      std::valarray<double> Ny1(0., particle.n_ygrid);
	      std::valarray<double> Ny2(0., particle.n_ygrid);
	      std::valarray<double> Ny3(0., particle.n_ygrid);
	      std::valarray<double> Ry(0., particle.n_ygrid);
...
#pragma omp parallel default(shared) private(iz,ix,iy,ip)
	    if (1 == galdef.prop_z) {
	      ...
	      std::valarray<double> Nz0(0., particle.n_zgrid);
	      std::valarray<double> Nz1(0., particle.n_zgrid);
	      std::valarray<double> Nz2(0., particle.n_zgrid);
	      std::valarray<double> Nz3(0., particle.n_zgrid);
	      std::valarray<double> Rz(0., particle.n_zgrid);
...
#pragma omp parallel default(shared) private(iz,ix,iy,ip)
	    if (1 == galdef.prop_p) { 
	      ...
	      std::valarray<double> Np0(0., particle.n_pgrid);
	      std::valarray<double> Np1(0., particle.n_pgrid);
	      std::valarray<double> Np2(0., particle.n_pgrid);
	      std::valarray<double> Np3(0., particle.n_pgrid);
	      std::valarray<double> Rp(0., particle.n_pgrid);
...
#pragma omp parallel for schedule(dynamic) private(ix,iy,iz,ip) default(shared)
...
#pragma omp parallel for schedule(dynamic)  private(ix,iy,iz,ip,ixyzp,iixyzp) default(shared)       //AWS20110202
