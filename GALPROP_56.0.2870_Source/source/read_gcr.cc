...
int Galprop::read_gcr() {
  ...
  buf<<"  reading  nuclei array from file "<<infile.str();
  INFO(buf.str());
  
  status = 0;         /* initialize status before calling fitsio routines */
  fits_open_file(&fptr, infile.str().c_str(), READONLY, &status);   
  buf.str("");
  buf<<"  fits open status = "<<status;
  INFO(buf.str());
  ...
  for (int i_species=0;i_species< n_species;i_species++){ 
    ...
    if(gcr[i_species].A!=0){
      ...
      buf.str("");
      buf<<"read NUCNORM status= "<<status<<"  NUCNORM="<<NUCNORM;
      INFO(buf.str());
    ...
    if(gcr[i_species].A==0&&gcr[i_species].Z==-1 && gcr[i_species].species == "knock_on_electrons"){
      fits_read_key(fptr,TDOUBLE,"NUCNORM" ,&NUCNORM ,comment,&status);  
      buf.str("");
      buf<<"read NUCNORM status= "<<status<<"  NUCNORM="<<NUCNORM;
      INFO(buf.str());
    ...
    if(gcr[i_species].A==0&&gcr[i_species].Z==-1 && gcr[i_species].species == "secondary_electrons"){
      fits_read_key(fptr,TDOUBLE,"NUCNORM" ,&NUCNORM ,comment,&status);  
      buf.str("");
      buf<<"read NUCNORM status= "<<status<<"  NUCNORM="<<NUCNORM;
      INFO(buf.str());
    ...
    // primary electrons
    if(gcr[i_species].A==0&&gcr[i_species].Z==-1 && gcr[i_species].species == "primary_electrons"){
      fits_read_key(fptr,TDOUBLE,"ELENORM" ,&ELENORM ,comment,&status);  
      buf.str("");
      buf<<"read ELENORM status= "<<status<<"  ELENORM="<<ELENORM;
      INFO(buf.str());
