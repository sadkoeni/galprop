...
//========================sigma_boron_dec_heinbach_simon
   if(galdef.cross_section_option>=100 
      && nuclei::sigma_boron_dec_heinbach_simon( iz, ia, jz, ja, 1000.)> 0.0)
   {
      if(galdef.verbose>=1)cout<<"     using Heinbach-Simon cross sections for ("<<iz<<","<<ia
         <<")  -> ("<<jz<<","<<ja<<")"<<endl;
      for(ip=0; ip<np; ip++)
      {
	sigma[ip]=nuclei::sigma_boron_dec_heinbach_simon( iz, ia, jz, ja, Ekin[ip]);
// here neglect difference for decaying channels
	nuclei::He_to_H_CS(Ekin[ip]/1000.,iz,ia,jz,ja,&CSratio,&CStot_ratio);
...
// skip if long-lived intermediate state
	    ...
	    nuclei::He_to_H_CS(Ekin[ip]/1000.,iz,ia,IZ1,IA1,&CSratio,&CStot_ratio);
