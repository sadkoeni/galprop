...
void Galprop::store_IC_emiss() {
  ...
  assert(2 == galaxy.n_spatial_dimensions || 3 == galaxy.n_spatial_dimensions);
  ...
  //Pick a location near the GC for the "radial" component.
  const size_t iY = (galaxy.n_spatial_dimensions == 2 ? 0 : galaxy.n_ygrid/2);
  ...
  long isotropicIC = 1;//(type == "isotropic" ? 1 : 0);
  ...
  for (unsigned int iComp = 0; iComp < axes[3]; ++iComp) {
	...
	for (unsigned int iR = 0; iR < axes[0]; ++iR) {
	  ...
	  if (iComp < axes[3] - 1) { // Individual components
	    ...
	    //if (type == "isotropic")
	    array[index] = (galaxy.n_spatial_dimensions == 2 ? galaxy.IC_iso_emiss[iComp].d2[iR][iZ].s[iP] : galaxy.IC_iso_emiss[iComp].d3[iR][iY][iZ].s[iP]);
	    //else
	    ...
	    for (unsigned int iC = 0; iC < axes[3]-1; ++iC)
	      //if (type == "isotropic")
		sum += (galaxy.n_spatial_dimensions == 2 ? galaxy.IC_iso_emiss[iC].d2[iR][iZ].s[iP] : galaxy.IC_iso_emiss[iC].d3[iR][iY][iZ].s[iP]);
	    //else
