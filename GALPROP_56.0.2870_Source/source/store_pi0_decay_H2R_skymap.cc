...
int Galprop::store_pi0_decay_H2R_skymap() {
  ...
  if (3 == galdef.skymap_format) {
    ...
    for (int i_Ring=0; i_Ring<galaxy.n_Ring; i_Ring++){
      ...
      filename += ".gz";
      SM::writeToFits(*galaxy.pi0_decay_H2R_hp_skymap[i_Ring], filename, true, true, "Energy", "MeV");
    }
    ...
    if(galdef.skymap_format == 1 || galdef.skymap_format == 2){ //!< Mapcube output compatable with Glast science tools
      ...
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], galaxy.pi0_decay_H2R_skymap.n_zgrid, galaxy.n_E_gammagrid, "pion_decay_H2R_mapcube_", true);
    }
