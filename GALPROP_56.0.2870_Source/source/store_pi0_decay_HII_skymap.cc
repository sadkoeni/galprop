...
int Galprop::store_pi0_decay_HII_skymap() {
  ...
  if (3 == galdef.skymap_format) {
    ...
    for (int i_Ring=0; i_Ring<galaxy.n_Ring; i_Ring++){
      ...
      filename += ".gz";
      SM::writeToFits(*galaxy.pi0_decay_HIIR_hp_skymap[i_Ring], filename, true, true, "Energy", "MeV");
    }
    ...
    naxes[2]=galaxy.pi0_decay_HIIR_skymap.n_zgrid; // number of Galactocentric rings
	...
	for    (int i_Ring   =0;        i_Ring <naxes[2];    i_Ring++)
	  for   (int ib       =0;            ib <naxes[1];        ib++)
	    for (int il       =0;            il <naxes[0];        il++)
	      {
		array[i]+=galaxy.pi0_decay_HIIR_skymap .d3[il][ib][i_Ring].s[ip] *pow(galaxy.E_gamma[ip],2);
		i++;
	      }
    ...
    if(galdef.skymap_format == 1 || galdef.skymap_format == 2){ //!< Mapcube output compatible with Glast science tools
	  ...
	  for   (int ib       =0;            ib <naxes[1];        ib++)
	    for (int il       =0;            il <naxes[0];        il++)
	      {
		array[i]=galaxy.pi0_decay_HIIR_skymap .d3[il][ib][i_Ring].s[ip];
		i++;
	      }
      
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], galaxy.pi0_decay_HIIR_skymap.n_zgrid, galaxy.n_E_gammagrid, "pion_decay_HII_mapcube_", true);
    }
