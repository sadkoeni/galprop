...
Distribution::Distribution(int n_rgrid, int n_zgrid, int n_pgrid) :
  n_pgrid(0), 
  n_rgrid(0), 
  n_zgrid(0), 
  n_xgrid(0), n_ygrid(0), 
...
Distribution::Distribution(int n_xgrid, int n_ygrid, int n_zgrid, int n_pgrid) :
  n_pgrid(0), 
  n_rgrid(0), 
  n_zgrid(0), 
  n_xgrid(0), n_ygrid(0), 
  ...
  d1(0), d2(0), d3(0) 
{
  ...
  init(n_xgrid, n_ygrid, n_zgrid, n_pgrid);
...
void Distribution::init(int n_rgrid_, int n_zgrid_, int n_pgrid_) {
   ...
   if ( n_spatial_dimensions != 2 || n_rgrid != n_rgrid_ || n_zgrid != n_zgrid_ || n_pgrid != n_pgrid_ ) {
  delete_array();
  n_spatial_dimensions=2;
  n_rgrid=n_rgrid_;
  n_zgrid=n_zgrid_;
  ...
  //For fast access on NUMA architecture
#pragma omp parallel for default(shared) schedule(static)
  for (int ir = 0; ir < n_rgrid; ++ir) {
    ...
    for (int iZ = 0; iZ < n_zgrid; ++iZ) {
       d2[ir][iZ].s.resize(n_pgrid,0.);
       d2[ir][iZ].nElems = n_pgrid;
      //d2[ir][iZ] = Spectrum(n_pgrid);
    }
...
Distribution::init(int n_xgrid_, int n_ygrid_, int n_zgrid_, int n_pgrid_) {
   ...
   if ( n_spatial_dimensions != 3 || n_xgrid != n_xgrid_ || n_ygrid != n_ygrid_ || n_zgrid != n_zgrid_ || n_pgrid != n_pgrid_ ) {
  delete_array();
  n_spatial_dimensions=3;
  n_xgrid=n_xgrid_;
  n_ygrid=n_ygrid_;
...
#pragma omp parallel for default(shared) schedule(static)
  for(int ix=0; ix<n_xgrid; ++ix) {
    ...
    for(int iy=0; iy<n_ygrid; ++iy) {
      ...
      d3[ix][iy]=new Spectrum [n_zgrid];
      for (int iz=0; iz<n_zgrid; ++iz) {
         d3[ix][iy][iz].s.resize(n_pgrid,0.);
         d3[ix][iy][iz].nElems = n_pgrid;
	//d3[ix][iy][iz] = Spectrum(n_pgrid);
      }
...
double Distribution::max() { //AWS20050624
  ...
  if (2 == n_spatial_dimensions)
    for(int ir=0; ir<n_rgrid; ir++)
      for(int iz=0; iz<n_zgrid; iz++)
	for(int ip=0; ip<n_pgrid; ip++)
	  max_ = (d2[ir][iz].s[ip] > max_ ? d2[ir][iz].s[ip] : max_);
  ...
  if (3 == n_spatial_dimensions)
	...
	for(int iz=0; iz<n_zgrid; iz++)
	  for(int ip=0; ip<n_pgrid; ip++)
	    max_ = (d3[ix][iy][iz].s[ip] > max_ ? d3[ix][iy][iz].s[ip] : max_);
...
Distribution Distribution::operator+(double const2) {
  ...
  if (2 == n_spatial_dimensions) {
    ...
    tmp.init(n_rgrid, n_zgrid, n_pgrid);
  ...
  if (3 == n_spatial_dimensions) {
    ...
    tmp.init(n_xgrid, n_ygrid, n_zgrid, n_pgrid); 
...
Distribution operator+(double const2,const Distribution &dist) {
  ...
  if (2 == dist.n_spatial_dimensions) {
    ...
    tmp.init(dist.n_rgrid, dist.n_zgrid, dist.n_pgrid);
  ...
  if (3 == dist.n_spatial_dimensions) {
    ...
    tmp.init(dist.n_xgrid, dist.n_ygrid, dist.n_zgrid, dist.n_pgrid);
