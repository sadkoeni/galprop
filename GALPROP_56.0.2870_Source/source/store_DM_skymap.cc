...
int Galprop::store_DM_skymap() {
  ...
  if (3 == galdef.skymap_format) {
    ...
    filename += ".gz";
    SM::writeToFits(*galaxy.DM_hp_skymap, filename, true, true, "Energy", "MeV");
	...
	for (int ib       =0;        ib<naxes[1];       ib++)
      ...
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], 1, galaxy.n_E_gammagrid, "DM_mapcube_", true);
    }
