...
int Galprop::store_pi0_decay_emiss() {
  ...
  for (int ip = 0; ip < naxes[2]; ++ip) {
	...
	const double emissivity = (2 == galaxy.n_spatial_dimensions ? galaxy.pi0_decay_emiss.d2[ir][iz].s[ip] : (3 == galaxy.n_spatial_dimensions ? galaxy.pi0_decay_emiss.d3[ir][0][iz].s[ip] : 0));
	...
	array[i] = emissivity*pow(galaxy.E_gamma[ip], 2.);
	
	//cout << ip << " " << iz << " " << ir << " " << i << " " << galaxy.pi0_decay_emiss.d2[ir][iz].s[ip] << " " << galaxy.E_gamma[ip] << " " << pow(galaxy.E_gamma[ip], 2.) << " " << galaxy.pi0_decay_emiss.d2[ir][iz].s[ip]*pow(galaxy.E_gamma[ip], 2.) << " " << array[i] << endl;
