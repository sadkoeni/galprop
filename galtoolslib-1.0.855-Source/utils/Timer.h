...
#ifndef _utl_Timer_h_
#define _utl_Timer_h_
...
#define TIME_SUBROUTINE(function, ...) \
  do {const size_t index = utl::Timer::GetInstance().start();	\
function(__VA_ARGS__); \
 utl::Timer::GetInstance().stop(index, #function, __FILE__, __LINE__);} while(0)
