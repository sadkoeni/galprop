...
c We should initialize all variables
      CS_S=0
      CS_SB=0
      CS_D=0
      CS_K=0
...
c KAON: electron/positron spectrum from pp -> K +X reaction
      if(iabs(key) .ge. 3 .and. iabs(key) .le. 4) then !KAON decay mode K->mu +nu
...
c inclusive cross section parameters
      if(key .eq. 0) then        ! GAMMA-RAYS
         ...
         C2 = 3.3
         C3 = 0.6
         Fp=(1.d0+23.d0/dsqrt(Pp**2+Mp**2)**2.6)*(1.d0-4.d0*Mp**2/s)**IR
      else
