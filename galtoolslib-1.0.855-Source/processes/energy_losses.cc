...
double nucleon_loss(int z, double MA, double emev, double nhcm3, 
   double nhicm3, double he_to_h,
   double* aion, double* coul) {

   double BK,PIR0H2C2,PIR02MC2C,Te,nhe_cm3,gam,bet,qmax,bh,bhe,
      coullog,bet_e,xm,we;

   if ( MA <= 0 ) {
      ERROR("Need an atomic number larger than 0");
      throw(std::invalid_argument("MA should be > 0"));
   }
...
// some constants
   ...
   if ( emev < MA ) {
      ERROR("Total energy should be larger than mass");
      throw(std::invalid_argument("emev too low"));
   }
...
main()
{
   ...
   {
      emev=a*(1.e3*Mp+Ek);
      nucleon_loss(z,1e3*a*amu,emev,nhcm3,nhicm3,he_to_h,&aion,&coul);
