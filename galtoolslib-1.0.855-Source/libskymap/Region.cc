...
SkyRegion::SkyRegion() {
  ftype = SkyRegion::DISC;
  fcenter = SM::Coordinate(0,0);
  fradius = 0;
}

SkyRegion::SkyRegion(SkyRegion::RegionType type, const SM::Coordinate &center, double radius, double deltal, double deltab) :
  ftype(type),
  fcenter(center) {
...
SkyRegion::SkyRegion(const SM::Coordinate &center, double deltal, double deltab) :
   SkyRegion(RECTANGLE, center, 0., deltal, deltab)
{}

SkyRegion::SkyRegion(const SM::Coordinate &center, double radius) :
   SkyRegion(DISC, center, radius, 0.0, 0.0)
{}

const SkyRegion::RegionType & SkyRegion::type() const { return ftype; }
...
std::istream & operator >> (std::istream &is, SkyRegion &region) {
  ...
  if (type == "DISC") {
    region.ftype = SkyRegion::DISC;
  } else if (type == "RECTANGLE") {
    region.ftype = SkyRegion::RECTANGLE;
  } else {
