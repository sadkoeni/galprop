...
class ArraySlice {
 ...
 public:
       ...
       */
  ArraySlice ( T *pointer, size_t size ) : fsize(size), fbegin(pointer), fbeginConst(pointer) {}
  ArraySlice ( const T *pointer, size_t size ) : fsize(size), fbegin(0), fbeginConst(pointer) {}
  ...
  template <typename C>
    ...
    if (fbegin == 0) {
      std::string errMsg = "Cannot assign to constant pointers";
