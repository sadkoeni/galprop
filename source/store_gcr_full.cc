...
int Galprop::store_gcr_full() {
  ...
  //Write only subsets of the image at a time.
  //Fits does not deal well with huge arrays
  ...
  //Compression can cause problems for large 3D calculations (large file sizes)
  ...
  //Do the last three axes at a time
  long nPixels = nAxes[2]*nAxes[1]*nAxes[0];
  valarray<float> array(nPixels);
  ...
  if (2 == gcr[0].n_spatial_dimensions) {
	...
	for (int iz = 0; iz < nAxes[1]; ++iz) {
      ...
      fits_write_img(fptr, TFLOAT, fpixel, nPixels, &array[0], &status);

      fpixel += nPixels;

      if (status != 0) {
         WARNING("Could not write fits file");
  ...
  for (int i_nucleus = 0; i_nucleus < n_species; ++i_nucleus) {
     ...
     sprintf(keyword,"NAME%03d",       i_nucleus+1 ); // e.g. NUCZ012
     sprintf(comment,"Name of nucleus %d",i_nucleus+1 ); 
     buf.str("");
     buf<<keyword<<" "<<gcr[i_nucleus].name;
     INFO(buf.str());
     fits_update_key(fptr, TSTRING, keyword , const_cast<char*>(gcr[i_nucleus].name.c_str()),comment, &status);
    ...
    sprintf(keyword,"NUCN%03d",       i_nucleus+1 ); // e.g. NUCN012                      
    sprintf(comment,"Normalization of nucleus %d",i_nucleus+1 );                            
    buf.str("");
    buf<<keyword<<" "<<gcr[i_nucleus].normalization_factor;
    INFO(buf.str());
    fits_update_key(fptr, TDOUBLE, keyword , &gcr[i_nucleus].normalization_factor,comment, &status);
