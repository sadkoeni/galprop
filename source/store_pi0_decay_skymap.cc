...
int Galprop::store_pi0_decay_skymap() {
  ...
  if (3 == galdef.skymap_format) {
    ...
    filename += ".gz";
    SM::writeToFits(*galaxy.pi0_decay_hp_skymap, filename, true, true, "Energy", "MeV");
	...
	(galdef.skymap_format == 2)) { //!< Mapcube output compatable with Glast science tools
      ...
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], 1, galaxy.n_E_gammagrid, "pion_decay_mapcube_", true);
    }
