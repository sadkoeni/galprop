...
int tridag(float  a[], float  b[], float  c[], float  r[], float  u[], int n) 
{
   ...
   //Declaring the variables static and threadprivate does not work for older versions of intel compilers.
#if defined(__INTEL_COMPILER) && __INTEL_COMPILER <= 1110
   double *gam = new double[n];
#else
         static int gam_size = 0;
	 static double *gam = 0;
#pragma omp threadprivate(gam,gam_size)
         if ( n > gam_size) {
            if (gam) delete[] gam;
	    gam = new double[n];
            gam_size = n;
...
#pragma omp threadprivate(gam,gam_size)
  ...
  for(auto j=n-2; j>=0; u[j]=u[j]-gam[j+1]*u[j+1],j--);
