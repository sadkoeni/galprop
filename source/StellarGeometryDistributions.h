...
namespace utl {
  ...
  public:
    virtual ~ArmFunction() {}
    virtual T operator()(const double r,
			 const double rScale,
			 const double rSun,
			 const double phi, 
			 const double z,
			 ...
			 const double armScale = 1.) = 0;
  };

  class ExponentialArmFunction : public ArmFunction<double> {
  ...
  public:
	...
	if (armPhi <= arm.fPhiMin + arm.fPhiExtent) {
	  ...
	  const double x = r*cos(phi), y = r*sin(phi), 
	    ...
	    armSigma = 0.5*arm.fWidth*armScale;///kSqrtTwo/kLogTwo*armScale;
	  
	  result = exp(-s/armSigma)*exp(-(r - rSun)/rScale - fabs(z)/zScale);
  ...
  class WainscoatArmFunction : public ArmFunction<double> {
  ...
  class NE2001ArmFunction : public ArmFunction<double> {
  ...
  public:
	...
	if (armPhi <= arm.fPhiMin + arm.fPhiExtent) {
	  ...
	  const double x = r*cos(phi), y = r*sin(phi), 
	    armX = r*cos(armPhi), armY = r*sin(armPhi), 
	    dX = armX - x, dY = armY - y, s = sqrt(dX*dX + dY*dY),
	    armSigma = arm.fWidth*armScale;

	  const double sech2R = (r > 11.5 ? pow(0.5/cosh(r - 11.5), 2.) : 1.);
	  const double sech2Z = pow(1./cosh(z/(0.25*zScale)), 2.);
	  
	  result = exp(-pow(s/armSigma, 2.))*sech2R*sech2Z;
  ...
  static double SDSSHalo(const double r, const double rSun, 
    ...
    return pow(rSun/sqrt(rp*rp + zOnQH*zOnQH), alpha);
  ...
  static double LopezCorredoiraBulgeAverage(const double r,
    ...
    //const double xpIndex = pow(r, 4.0), ypIndex = pow(0/0.49, 4.0), 
    //zpIndex = pow(z/0.37, 4.0);

    //const double t = pow(xpIndex + ypIndex + zpIndex, 1.0/4.0);

    //cout << r << " " << phiSteps << " " << result << " " << result/phiSteps << endl;
  ...
  static double ExponentialArm(const double r,
			       const double rScale,
			       const double rSun,
			       const double phi, 
			       const double z,
	...
	const double x = r*cos(phi), y = r*sin(phi), 
	  armX = r*cos(armPhi), armY = r*sin(armPhi), 
	  dX = armX - x, dY = armY - y, s = sqrt(dX*dX + dY*dY),
	  armSigma = 0.5*arm.fWidth/kSqrtTwo/kLogTwo*armScale;

	result = exp(-s/armSigma) //exp(-0.5*pow(s/(kSqrtTwo*armSigma), 2.))
	  *exp(-(r - rSun)/rScale - fabs(z)/zScale);
  ...
  static double ExponentialArmAverage(const double r,
				      const double rScale,
				      const double rSun,
				      const double z,
				      const double zScale,
    ...
    for (unsigned int i = 0; i < phiSteps; ++i) {
      ...
      result += ExponentialArm(r, rScale, rSun, phi, z, zScale, arm);
