...
int Galprop::gen_bremss_emiss()  {
  ...
  for (i=0, ielectrons=-1; i<n_species; i++)  
    if (100 == 100*abs(gcr[i].Z)+gcr[i].A) {
      ...
      ostringstream os;
      os<<"  CR "<<gcr[ielectrons].name<<" found as species #"<<ielectrons;
      INFO(os.str());
  ...
  if(ielectrons==-1) { 
     ostringstream os;
     os<<"CR electrons/positrons not found!"; 
     INFO(os);
...
#pragma omp parallel for schedule(dynamic) default(shared) private(cs_HII,cs_HI,cs_He,cs,y,ye,fe,Ne1,IZ1,key,i) firstprivate(Ekin)
  for (int iEgamma = 0; iEgamma < galaxy.n_E_gammagrid; ++iEgamma) {
	...
	cs_HII = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip],IZ1,Ne1);
	
	IZ1=1; Ne1=1; //              1-electron atom  H I
	cs_HI = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip],IZ1,Ne1);
            
	IZ1=2; Ne1=2; //              2-electron atom  He
	cs_He = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip],IZ1,Ne1);
	...
	IZ1=1; Ne1=0; //              ionized hydrogen H II
	cs[1][0] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip+1],IZ1,Ne1);
            
	IZ1=1; Ne1=1; //              1-electron atom  H I
	cs[1][1] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip+1],IZ1,Ne1);

	IZ1=2; Ne1=2; //              2-electron atom  He
	cs[1][2] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],gcr[ielectrons].Etot[ip+1],IZ1,Ne1);
// fool proof
	if (cs[1][1] <= 0.) 
	  continue;
	...
	if (cs[0][1] <= 0. && cs[1][1] > 0.) {
	  ...
	  IZ1=1; Ne1=0; //              ionized hydrogen H II
	  cs[0][0] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],Ekin+Mele,IZ1,Ne1);
               
	  IZ1=1; Ne1=1; //              1-electron atom  H I
	  cs[0][1] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],Ekin+Mele,IZ1,Ne1);
               
	  IZ1=2; Ne1=2; //              2-electron atom  He
	  cs[0][2] = processes::bremss_spec_cc(galaxy.E_gamma[iEgamma],Ekin+Mele,IZ1,Ne1);
...
// derive y (= power-law index of the total expression)
		...
		if (std::isnan(val) || std::isinf(val)) {
                   ostringstream os;
		  os<<y<<", "<<cs[0][1]<<", "<<cs[0][2]<<", "<<cs[1][1]<<", "<<cs[1][2]<<", "<<fe<<", "<<electrons.d2[ir][iz].s[ip+1]<<", "<<Ekin<<", "<<gcr[ielectrons].Ekin[ip+1];
                  ERROR(os.str());
		}
		...
		if (electrons.d3[ix][iy][iz].s[ip  ]>0.0&&  electrons.d3[ix][iy][iz].s[ip+1]>0.0 ) //AWS20100503
		  {
		    ...
		    /*
		      if(std::isnan( galaxy.bremss_emiss.d3[ix][iy][iz].s[iEgamma] ))   //AWS20100503
