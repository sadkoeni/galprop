...
int Galprop::nuclei_normalize() {
  ...
  if ("Hydrogen_1" != gcr[iprotons].name) //strcmp(gcr[iprotons].name,"Hydrogen_1")!=0) 
    { WARNING("CR protons not found!"); return 1; }
  ostringstream buf;
  ...
  const int ip = int(log(galdef.proton_norm_Ekin/galdef.Ekin_min)/log(galdef.Ekin_factor) + 0.5);//IMOS20060420
  const int iz = int((-galdef.z_min)/galdef.dz + 0.5);//IMOS20060420

  if (ip < 0 || ip >= gcr[iprotons].n_pgrid) {
     ERROR("Proton normalization energy out of bounds, aborting normalization");
     return 1;
  }
  if (iz < 0 || iz >= gcr[iprotons].n_zgrid) {
  ...
  if (2 == galdef.n_spatial_dimensions) {
    ...
    if (ir < 0 || ir >= gcr[iprotons].n_rgrid) {
  ...
  if (3 == galdef.n_spatial_dimensions) {
    ...
    if (ix < 0 || ix >= gcr[iprotons].n_xgrid) {
       ERROR("Solar position not within grid, aborting normalization");
       return 1;
    }
    if (iy < 0 || iy >= gcr[iprotons].n_ygrid) {
       ERROR("Solar position not within grid, aborting normalization");
       return 1;
    }
  ...
  const double vnorm = exp(log(v5) + log(galdef.proton_norm_Ekin/gcr[iprotons].Ekin[ip])/log(galdef.Ekin_factor)*log(v6/v5));
  ...
  if (v5 <= 0 || v6 <= 0) {
     ERROR("Proton spectrum 0 at normalization point, aborting normalization.");
  ...
  for (int i = 0; i < n_species; i++) {
    if ("primary_electrons" == gcr[i].name) 
      continue; //strcmp(gcr[i].name,"primary_electrons")==0) continue;// IMOS20050912
    if ("primary_positrons" == gcr[i].name) 
      continue; //strcmp(gcr[i].name,"primary_electrons")==0) continue;// IMOS20050912

    // don't re-normalize the DM annihilation products
    if ("DM_positrons" == gcr[i].name)
      continue; //strcmp(gcr[i].name,"DM_positrons"  )==0) continue;  // IMOS20050912
    if ("DM_electrons" == gcr[i].name)
      continue; //strcmp(gcr[i].name,"DM_electrons"  )==0) continue;  // IMOS20050912
    if ("DM_antiprotons" == gcr[i].name)
      continue; //strcmp(gcr[i].name,"DM_antiprotons")==0) continue;  // IMOS20050912
