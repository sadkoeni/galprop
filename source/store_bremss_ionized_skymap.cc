...
int Galprop::store_bremss_ionized_skymap() {
  ...
  if (galdef.skymap_format == 3) {
    ...
    filename += ".gz";
    SM::writeToFits(*galaxy.bremss_ionized_hp_skymap, filename, true, true, "Energy", "MeV");
	...
	for (int ib       =0;        ib<naxes[1];       ib++){
      ...
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], 1, galaxy.n_E_gammagrid, "bremss_ionized_mapcube_", true);
    }
