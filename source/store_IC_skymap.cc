  ...
  if (3 == galdef.skymap_format) {
    ...
    if (IC_type== "isotropic"){                             //AWS20090415
      //Add all the maps to an initial map
       std::unique_ptr< SM::BaseSky<double> > outmap = galaxy.createFullSky(galdef.healpix_order);
      for (int i_comp=0; i_comp<galaxy.n_ISRF_components; i_comp++){
         SM::ApplyFunction<double,double>(*outmap, *galaxy.IC_iso_hp_skymap[i_comp], [](double &a, const double &b) { a += b; });
      }
      SM::writeToFits(*outmap, filename, true, true, "Energy", "MeV");
    }
    ...
    if(       IC_type=="anisotropic" ){                               //AWS20090415
      //Add all the maps to an initial map
       std::unique_ptr< SM::BaseSky<double> > outmap = galaxy.createFullSky(galdef.healpix_order);
      for (int i_comp=0; i_comp<galaxy.n_ISRF_components; i_comp++){
         SM::ApplyFunction<double,double>(*outmap, *galaxy.IC_aniso_hp_skymap[i_comp], [](double &a, const double &b) { a += b; });
      }
      SM::writeToFits(*outmap, filename, true, true, "Energy", "MeV");
    }
	...
	for (int ib=0; ib<naxes[1]; ib++)
      ...
      status = store_mapcube_skymap(&array[0], &galaxy.E_gamma[0], 1, galaxy.n_E_gammagrid, filename+"mapcube_", true);
    }
