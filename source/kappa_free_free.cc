...
std::valarray<double> KappaFreeFree::operator () ( const double x, const double y, const double z, const vec3 &dir ) const {
  std::valarray<double> out(nu);
  for (size_t i = 0; i < nu.size(); ++i) {
    out[i] = kff(nu[i], nHII3D(x,y,z), Te)*clumping_factor;
  }
  return out;
}

std::valarray<double> EmissFreeFree::operator () ( const double x, const double y, const double z, const vec3 &dir ) const {
  std::valarray<double> out(nu);
  for (size_t i = 0; i < nu.size(); ++i) {
    out[i] = eff(nu[i], nHII3D(x,y,z), Te)*clumping_factor;
  }
  return out;
}
